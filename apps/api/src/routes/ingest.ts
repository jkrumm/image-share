...
// Agent ingest (design §8). Multipart upload lands in the service-owned uploads
// area (UPLOADS_DIR/<yyyy>/<mm>/, collision-safe name), is indexed immediately,
// and returns the canonical admin serving URL.
...
export const ingestRoutes = new Elysia({ name: 'ingest' }).post(
  ...
  async ({ body, set }) => {
    ...
    const dirAbs = join(env.UPLOADS_DIR, dirRel)
  ...
  },
  {
    body: z.object({
      ...
      dir: z.string().optional().describe('Optional sub-directory hint under the uploads area'),
    }),
    response: {
      201: z.object({
        id: z.number().int(),
        root: z.literal('uploads'),
        relPath: z.string(),
    ...
    detail: {
      ...
      description:
        'Multipart upload of a single image into the service-owned uploads area (UPLOADS_DIR/<yyyy>/<mm>/ with a collision-safe name). Indexes the file immediately and returns its id, relPath, and the admin serving URL. This is the private ingest path — public publishing is POST /api/publish.',
      security: [{ BearerAuth: [] }],
    },
  },
)
